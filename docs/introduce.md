...
前端大神Anthony Fu在自己的博客[《聊聊纯 CSS 图标》](https://antfu.me/posts/icons-in-pure-css-zh)中提到：**在纯 CSS 中按需使用任何图标的能力**的一套方案，能支持单色和彩色图标，并且他将这个图标方案集成在了`UnoCSS`的[图标库预设](https://unocss.dev/presets/icons)中。在当前组件化思想中，很多框架提供了组件图标的概念，但是组件化的图标会带来一定的性能影响，最好的方案还是`纯CSS`的方案，Anthony Fu大神发表过`组件库不适合做图标`的意见，让我们更加坚定用`CSS`来做图标的想法。
